---
title: "Do the Warriors *really* play the right way?"
...
<img src = "C:\\Users\\fielrg17\\Google Drive\\MSBA Classses\\Intro to R\\nba-stats-logo.svg" height = 125 width = 400 align = right>
...
More importantly, the NBA is on the cutting-edge of data collection in sports. Thanks to the NBA's partnership with [STATS SportVU](https://www.stats.com/sportvu-basketball-media/), they are tracking data well beyond the box score most of us have seen in a newspaper. SportVU is a system of 6 cameras hung from the ceiling that track ball and player movement 25 times per second to produce metrics such as secondary assists, miles traveled, made field goals defended at rim, and one frequency on pick and rolls, etc.
...
Lucky for us, the NBA has made the data they collect readily available on their [website](stats.nba.com). Unfortunately, they do not make it readily available for download. This required some data wranglin' using the `jsonlite` package.
...
* How am I defining making your teammates 'better'?
    - I'll look at Real Plus/Minus while a particular player is on the court. This captures both an offensive and defensive perspective.
* How will I measure a player's impact on this definition of 'better'?
    - I will try to extract a player's direct impact on his team's Real Plus/Minus (i.e. points he scores or allows himself)
* And how do I determine the difference between making teammates better and a synergistic effect of particular teammates.
    - Comparing similar lineups with and without a particular player may shed light on this, but ultimately this question may not matter. The information is still useful in picking lineups that maximize a group of players' value.
...
### Packages Used
```{r packages, echo = TRUE, message = FALSE, include = TRUE, warning = FALSE}
  # Packages Required
...
I dropped any unneccessary variables (the NBA JSON files provided separate variables to rank all the included variables, etc.). I started with a couple hundred variables, but pared this down to 39 variables for **Team** data and 67 variables for **Player** data that might be of interest later on.

I have included a data dictionary with details on what each variable is. A more in depth look at different statistics used for NBA data can be found in [the NBA stats glossary](stats.nba.com/help/glossary).

```{r nba_data_import, echo = TRUE, message = FALSE, warning = FALSE}
...
datatable(player_data,
          ...
          options = list(orderClasses = TRUE,
                         ...
                         orderMulti = TRUE,
                         columnDefs =
                           list(list(visible = FALSE, targets = c(1,3)))))
...
datatable(team_data,
          ...
          options = list(orderClasses = TRUE,
                         ...
                         orderMulti = TRUE,
                         columnDefs =
                           list(list(visible = FALSE, targets = 1))))
...
In thinking through how I wanted to approach my analysis, I settled on two different questions:
  1. How do teams perform against a baseline of being the "sum of their parts"?
...
I plotted both of these with a line with slope = 1 because my null hypothesis is that teams are equal to the sum of their parts. I also plotted these with a `geom_smooth` line of best fit to show how teams might compare to other teams in the league.
...
I wasn't quite able to quantify which players were responsible for a team under or overperforming, running into some of the same issues that Plus/Minus has, that it's difficult to distinguish between a player helping his teammates or a player being helped *by* his teammates.

Ultimately, I'd like to build some a model that looks into what players do on the court (i.e. shooting, passing, defensive statistics, etc.) and how this contributes to the synergy of a team. Furthermore, I'd like to see which player lineups are most effective at pushing a team past its "sum of its parts" baseline.

I'm disappointed I didn't get further in this analysis, but this is a project I will continue to build on in my free time.
